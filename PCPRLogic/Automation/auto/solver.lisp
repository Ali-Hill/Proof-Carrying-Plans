...
(load "../auto/domain_lisp")
(load "../auto/problem_lisp")
...
(defun planToString (plan)
  ...
  (concatenate 'string "plan = " st " shrink)")
))
...
(with-open-file ( my-stream (concatenate 'string outputfile ".agda") :direction :output :if-exists :append )
  ...
  (write-line "Derivation = weakening P (from-yes (decSub P P')) tt (composition " my-stream)

  (setq finalSub (car postSub))

  (setq i 0)
  (loop for p in (cdr postSub)
      do (progn
          (write-line (concatenate 'string sp8 "(weakComp (from-yes (decSub (" p ")") my-stream)
          (write-line (concatenate 'string sp8 "(" (nth i preSub) ")))") my-stream)
          (incf i)
        )
  )
  ...
  (write-line (concatenate 'string sp8 "(shrink tt tt (from-yes (decSub (" finalSub ") Q))))") my-stream)
